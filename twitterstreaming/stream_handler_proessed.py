####################################################################################################################
#
# File:        stream_handler_processed.py
# Description: Same as stream_handler.py, but it keeps tracking offsets, and persist them in disk,
#              to make sure that all the data in the topic is consumed by spark, this file is used
...
class StreamHandler(object):
    ...
    # save last consumed offset for every rdd
    def save_offset(self, rdd, offset_file_path):
        offset_ranges = rdd.offsetRanges()
        with open(offset_file_path, 'w') as f:
            pickle.dump(offset_ranges, f)
    ...
    def process(self, rdd, spark, offset_file_path):
        # get desired fields
        ...
        if (not rdd_fields.isEmpty()):
            ...
            # since processing is done, then save last consumed offset
            self.save_offset(rdd, offset_file_path)
